...
# Introduction
We developed CEDA is developed to analyze read counts of single guide RNAs (sgRNAs)
rom CRISPR screening experiments. The sgRNAs are synthetically generated from genes, 
...
model fit by an EM algorithms. Posterior estimates at sgRNA level are then summarized 
for each gene.
...
## Data Format
In our exoperiment, three samples of MDA231 cells were untreated at time T=0,
and another three samples of MDA231 cells were treated with DMSO at time T=0.
We are interested  in detecting sgRNAs that are differentially changed by treatment. 

The sgRNA read counts, along with a list of non-essential genes, are stored 
in the dataset `mda231` that we have bnicluded in the `CEDA` package. We read that
...
1. `sgRNA`, the observed count data, and
...
head(mda231$sgRNA)
```
Notice that this includes an extra column, "`exp.level.log2`", that was computed from
the raw data.
...
The second element of the list is, as expected, just a list of gene names:
```{r neGene}
dim(mda231$neGene)
head(mda231$neGene)
```
